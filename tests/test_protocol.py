...
def test_tldr_returns_response(llm_params, monkeypatch):
    client = make_stub_openai_client()
    paper = make_sample_paper()
    monkeypatch.setattr(paper, "_generate_tldr_from_semantic_scholar", lambda: None)
    result = paper.generate_tldr(client, llm_params)
    assert result == "Hello! How can I assist you today?"
    assert paper.tldr == result


def test_tldr_without_abstract_or_fulltext(llm_params, monkeypatch):
    client = make_stub_openai_client()
...
def test_tldr_falls_back_to_abstract_on_error(llm_params, monkeypatch):
    paper = make_sample_paper()
    monkeypatch.setattr(paper, "_generate_tldr_from_semantic_scholar", lambda: None)
...
def test_tldr_truncates_long_prompt(llm_params):
    ...
    paper._generate_tldr_from_semantic_scholar = lambda: None
    result = paper.generate_tldr(client, llm_params)
    assert result is not None


def test_tldr_prefers_semantic_scholar_when_available(llm_params, monkeypatch):
    from types import SimpleNamespace

    paper = make_sample_paper()
    monkeypatch.setattr(paper, "_generate_tldr_from_semantic_scholar", lambda: "Semantic Scholar TLDR")

    broken_client = SimpleNamespace(
        chat=SimpleNamespace(
            completions=SimpleNamespace(create=lambda **kw: (_ for _ in ()).throw(RuntimeError("should not call llm")))
        )
    )

    result = paper.generate_tldr(broken_client, llm_params)
    assert result == "Semantic Scholar TLDR"
    assert paper.tldr == "Semantic Scholar TLDR"
